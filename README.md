...
**CC BY-NC 4.0**（姓名標示 — 非商業性）＋**方睿額外授權**。白話講：

| | |
|---|---|
| ✅ **自己用、給同事用、改成你們團隊的版本** | 免費，不用問我們 |
| ✅ **公司內部導入、內部訓練、改善你們自己的工作流程** | 免費——你們公司是營利企業也一樣 |
| ✅ **分享、轉貼、寫成文章** | 請註明出處：方睿科技 FUNRAISE，並附上這個 repo 的連結 |
| ⚠️ **改過之後又對外發布** | 移除方睿的官方頁尾，改標「Based on context-interview by 方睿科技 FUNRAISE；本版本已經修改」 |
| ⚠️ **商業使用**（收費課程、顧問交付物、包進你的產品、對外販售） | 需事前簽署書面商業授權契約 |

「公司內部導入免費」不是靠 NC 條款的解釋撐著，是方睿**另外給的一層授權**，條款寫在 [`NOTICE.md`](NOTICE.md) 第五節。完整條文見 [`LICENSE`](LICENSE)。

**開始之前請先確認一件事**：要餵給 AI 的如果是公司內部資料、員工或客戶個資、未公開財務資料或營業秘密，先確認你有權使用、而且你選的那個 AI 服務符合貴公司的個資與資安政策；必要時先去識別化。這個專案不設方睿的資料接收或儲存機制、不會把任何資料送到方睿的伺服器——但資料會進到**你自己選的那個 AI**，那一段取決於你和它之間的服務條款。詳見 [`NOTICE.md`](NOTICE.md) 第六節。
